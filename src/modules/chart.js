...
export const renderChart = (data) => {
  // data.forEach((patient) => {
    ...
    const createChart = (patient, months) => {
      ...
      const systolicData = filteredHistory.map(item => item.blood_pressure.systolic.value);
      const diastolicData = filteredHistory.map(item =>item.blood_pressure.diastolic.value);
